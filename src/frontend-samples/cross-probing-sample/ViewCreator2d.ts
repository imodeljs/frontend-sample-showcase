...
export class ViewCreator2d {
  ...
  private static drawingModelClasses = [DrawingModelState.classFullName, SectionDrawingModelState.classFullName];
  private static sheetModelClasses = [SheetModelState.classFullName];
  ...
  /**
   * Checks to see if given model is a Drawing Model.
  ...
  /**
   * Checks to see if given model is a Sheet Model.
   * @param modelType classFullName of model.
   */
   ...
   */
  public async getViewForModel(modelId: Id64String, modelType: string, options?: ViewCreator2dOptions): Promise<ViewState> {
  ...
  private async _mergeSeedView(modelId: Id64String, props: ViewStateProps): Promise<ViewStateProps> {
    ...
    const seedViewState = (await this._imodel.views.load(viewDefinitionId)) as ViewState2d;
