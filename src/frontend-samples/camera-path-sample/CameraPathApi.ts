...
export default class CameraPathApi {
...
export class CameraPath {
  ...
  private _getDirection(point: Point3d) {
    ...
    const numPoints = lineString.packedPoints.length - 1;
    // as we drop one above it seems that we cannot use the last point
    ...
    if (numPoints - 1 === segmentIndex)
      return new Point3d(this._directions[segmentIndex].x, this._directions[segmentIndex].y, this._directions[segmentIndex].z);
